...
export default function Gift({ gift, onNextSlideClick }: GiftProps) {
    ...
    const selectedScaleClass = cardData && selectedBudget.includes(cardData.gift) ? 'scale-110' : '';
    return (
        <div className={`hover-supported:hover:cursor-pointer transform z-0 mx-auto
                ...
                ${selectedScaleClass}
            `}
            onClick={() => {dispatch(changeBudget(gift)); onNextSlideClick()}}
        >
