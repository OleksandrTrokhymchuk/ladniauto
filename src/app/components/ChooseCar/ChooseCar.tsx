...
export default function ChooseCar() {
  ...
  const [isAllOptionsSelected, setIsAllOptionsSelected] =
    useState<boolean>(false);

  const selectedCarBody = useAppSelector(
    (state) => state.selectedCarBodies.value
  );
  const selectedFuelType = useAppSelector(
    (state) => state.selectedFuelTypes.value
  );
    ...
    try {
      const result = await emailjs.send(
        ...
        TEMPLATE_ID,
        {
          ...
          user_carBudgetInfo: "Не вказано",
          user_carBody: selectedCarBody,
          user_fuelType: selectedFuelType,
          user_budget: `${selectedBudget} $`,
          user_gift: selectedGift,
          user_service: "Не вказано",
          user_callTime: "Не вказано",
        },
        PUBLIC_KEY
      );
          ...
          <div className="vsm:grid-small-auto-fit-cards msm:grid-auto-fit-cards">
            {carBodies.map((carBody) => (
              <CarBody
                key={carBody}
                carBody={carBody}
                onNextSlideClick={handleNextSlide}
              />
            ))}
          </div>
        </SwiperSlide>
